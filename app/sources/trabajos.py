...
class TrabajosJobSource:
    name = "trabajos"

    def __init__(self, settings: dict[str, Any]) -> None:
        self.enabled = bool(settings.get("enabled", True))
        self.base_url: str | None = str(settings.get("base_url", "https://www.trabajos.com"))
        ...
        async with httpx.AsyncClient(
            ...
            for page in self.pages:
                try:
                    ...
                    logger.warning(
                        "No se pudo consultar la pagina de Trabajos.com",
    ...
    async def _fetch_page(self, client: httpx.AsyncClient, page: TrabajosPage) -> str:
        ...
        for attempt in range(self.retries + 1):
            try:
                ...
                logger.warning(
                    "Fallo temporal consultando Trabajos.com",
                    extra={"source": self.name, "url": page.url, "attempt": attempt + 1},
                )
                await asyncio.sleep(wait_seconds)
        raise RuntimeError(f"Trabajos.com page failed after retries: {last_error}") from last_error

    def _jobs_from_html(self, payload: str, page: TrabajosPage) -> list[RawJob]:
        soup = BeautifulSoup(payload, "html.parser")
        jobs: list[RawJob] = []
        for card in soup.select("div.listado2014.card.oferta"):
            job = self._to_raw_job(card, page)
            if job is not None:
                jobs.append(job)
        return jobs

    def _to_raw_job(self, card: Tag, page: TrabajosPage) -> RawJob | None:
        link = card.select_one("a.oferta")
        ...
        if title is None or href is None:
            return None
        company = _text(card.select_one("a.empresa span")) or "Unknown company"
        description = _text(card.select_one(".doextended")) or title
        meta = _text(card.select_one("p.oi"))
        location = _text(card.select_one(".location")) or page.location_hint
        combined = " ".join(part for part in (title, description, meta or "") if part)
        return RawJob(
            source_name=self.name,
            source_job_id=_optional_str(link.get("data-j4m_val")) or _source_job_id(href),
            ...
            url=href,
            publication_date=_parse_date(_text(card.select_one(".fecha"))),
            raw_payload={
                "page_url": page.url,
                "title": title,
                "company": company,
...
def _page_list(value: object) -> list[TrabajosPage]:
    if not isinstance(value, list):
        return []
    pages: list[TrabajosPage] = []
    for item in value:
...
def _source_job_id(href: str) -> str:
    match = re.search(r"/ofertas/(?P<id>\d+)/", href)
    if match:
...
def _employment_type(meta: str | None) -> str | None:
    ...
    if "media jornada" in folded or "jornada parcial" in folded:
        return "part_time"
    if "jornada intensiva" in folded:
        return "full_time"
    return None


def _salary_text(meta: str | None) -> str | None:
    if not meta or not re.search(r"(?:€|eur|euro)", meta, re.IGNORECASE):
        return None
    return meta
